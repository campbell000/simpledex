...
/*
 ...
 NSString* oldProse;
 NSNumber* oldGenID;
 NSNumber* oldMoveID = [NSNumber numberWithInt:9];
 NSNumber* oldLangID;
 ...
 NSNumber* genID = [NSNumber numberWithInt:[[row objectAtIndex:1]intValue]];
 NSNumber* langID = [NSNumber numberWithInt:[[row objectAtIndex:2]intValue]];
 
 NSString* prose = [row objectAtIndex:3];
 
 //Only add most recent, english descriptions. Fuck those japs.
 if ([oldLangID intValue] == 9 && (oldGenID > genID || oldMoveID < moveID))
 {
 [moveProse setObject:oldProse forKey:moveID];
 if ([moveID intValue] - [oldMoveID intValue] > 1)
 {
 NSLog(@"The move after %@ has no desc", oldMoveID);
 }
 }
 oldGenID = genID;
 oldProse = prose;
 ...
 * english description that it sees. This could be most easily fixed by making adjustments to the csv file. It's
 * difficult to do this through code because
 *
 * 1. The "Most Recent" descriptions aren't necessarily all from the same generation. So I can't simply say get all 
 *    english descriptions from gen 15.
...
- (void) createMoveDescDictionary
{
    ...
    for (int i = 1; i < [rows count]; i++)
    {
        ...
        //Right now, this algorithm does not add the most recent enlgish description. Instead, it just adds the first english description that it sees. This could be most easily fixed by making adjustments to the csv file.
